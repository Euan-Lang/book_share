...
def user_search(request):
    ...
    if request.is_ajax and request.method == "POST": 
        ...
        return JsonResponse({"instances":[{"username":result.user.username, "image_url": result.user_image.url if result.user_image else "none"} for result in results]}, status=200)
